  ...
  <div class="about">
   ...
   <div class="search-songs">
    <van-list v-model="loading" :finished="finished" @load="onLoad">
      <van-cell v-for="song in songs" :key="song.id" :label="song.artists" @click="addToSongs(song.id)">
  ...
  methods: {
    async onSearch(offset) {
      ...
      try {
        ...
        this.loading = false;
        this.songs.length === this.songCount ? (this.finished = true) : null;
      } catch (err) {
        console.log(err);
      }
    },
    ...
    async init(songId) {
      try {
        ...
        const flag = this.audio.some(item => {
